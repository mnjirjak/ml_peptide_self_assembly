import numpy as np
from output_thr import predict_for_validation, read_PR_ROC
from automate_training import hyperparameter_numcells, hyperparameter_kernel_size
...
for train_and_validation_data_indices, test_data_indices in kfold_first.split(all_data, all_labels):
    ...
    # Train the model.
    predict_for_validation(model_name, test_number, train_and_validation_data, train_and_validation_labels, kfold_second)

for numcells in hyperparameter_numcells: 
    for kernel in hyperparameter_kernel_size:   
        read_PR_ROC(model_name, numcells, kernel)